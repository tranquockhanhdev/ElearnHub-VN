...
const handleOpenEditModal = (category) => {
  ...
  const handleEditSubmit = (e) => {
    e.preventDefault();
    router.put(route('admin.course-category.update', editData.id), editData, {
      onSuccess: () => {
        ...
        <div className="row mb-3">
          <div className="col-12 d-sm-flex justify-content-between align-items-center">
            <h1 className="h3 mb-2 mb-sm-0">
              Course Categories{' '}
              <span className="badge bg-orange bg-opacity-10 text-orange">{categories.length}</span>
            </h1>
              <button onClick={handleOpenCreateModal} className="btn btn-sm btn-primary mb-0">
              Create a Category
            </button>
          </div>
        </div>
                ...
                <tbody>
                  {categories.map((category, index) => (
                    <tr key={category.id}>
                      ...
                      <td>{category.slug}</td>
                      <td>
                        <span className={`btn-sm rounded-pill px-2 py-1 fw-semibold 
                          ${category.status === 'active' ? 'bg-success text-white' : ''}
                          ${category.status === 'inactive' ? 'bg-secondary text-white' : ''}
                          ${category.status === 'suspended' ? 'bg-danger text-white' : ''}`}>
...
>
                        ...
                        <button className="btn btn-sm btn-danger" onClick={() => confirmDelete(category.id)}>
                          Delete
                        </button>
                      </td>
                    </tr>
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!manualSlug) {
                          const generatedSlug = slugify(e.target.value, { lower: true, strict: true });
                          setFormData((prev) => ({ ...prev, slug: generatedSlug }));
                        }
                      }}
                    />
{localErrors.name && <div className="invalid-feedback">{localErrors.name}</div>}
                  ...
                  <div className="mb-3">
                    ...
                    />
                   {localErrors.slug && <div className="invalid-feedback">{localErrors.slug}</div>}
                  </div>
